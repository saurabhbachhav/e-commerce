...
const Login: React.FC = () => {
  ...
  const handleSocialLogin = (provider: string) => {
    if (provider === "google") signIn("google", { callbackUrl: "/" });
    else alert("Only Google login is available for now.");
  };

  return (
    <div className={` flex flex-col md:flex-row min-h-screen bg-gradient-to-r ${
      darkMode ? "from-gray-800 via-gray-900 to-black" : "from-blue-50 via-white to-blue-100"
    }`}>
      {/* Form Panel */}
      <motion.div
        ...
        className={`w-full md:w-1/3 flex flex-col items-center p-6 sm:p-8 lg:p-12 bg-gradient-to-b ${
        ...
        <div className="w-full max-w-md mt-16 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <p className="text-center text-sm">
              ...
              <Link href="/signup" className={`hover:underline ${darkMode ? "text-blue-300" : "text-blue-600"}`}>Sign up</Link>
            </p>
          </form>
            ...
            {socialProviders.map(({ icon: Icon, name }) => (
              <motion.button
                ...
                className={`flex items-center justify-center space-x-2 px-4 py-3 min-w-[100px] rounded-full shadow-lg bg-gradient-to-tr ${
                  name === 'google' ? 'from-red-400 to-red-600' : name === 'facebook' ? 'from-blue-600 to-blue-800' : 'from-pink-500 to-pink-700'
                } text-white hover:brightness-110 active:brightness-90`}
              >
                <Icon size={20} />
                <span className="text-sm sm:text-base font-semibold capitalize">{name}</span>
              </motion.button>
            ))}
          </div>
        </div>
